...
Base_packages=(nvidia-driver-525 curl util-linux apt-transport-https wget gpg htop x11-utils udisks2 jq mlocate lxqt-sudo pavucontrol cmake nginx xclip)
...
apt_install() {
    ...
    if [[ $external == "True" ]]
    then
        echo "apt install -y ${install_packages[@]}"
